module Groupdate
  class RelationBuilder
    attr_reader :period, :column, :day_start, :week_start, :time_offset

    def initialize(relation, column:, period:, time_zone:, time_range:, week_start:, day_start:, time_offset:)
      @relation = relation
      @column = column
      @period = period
      @time_zone = time_zone
    ...
    def group_clause
      ...
      query =
        ...
        when "MySQL", "Mysql2", "Mysql2Spatial", 'Mysql2Rgeo'
          case period
          when :day_of_week
            ["DAYOFWEEK(CONVERT_TZ(DATE_ADD(DATE_SUB(#{column}, INTERVAL #{day_start} second),INTERVAL '#{time_offset}' SECOND), '+00:00', ?)) - 1", time_zone]
          when :hour_of_day
        ...
        when "PostgreSQL", "PostGIS"
          ...
          when :week # start on Sunday, not PostgreSQL default Monday
            ["(DATE_TRUNC('#{period}', (#{column}::timestamptz - INTERVAL '#{week_start} day' - INTERVAL '#{day_start} second' + INTERVAL '#{time_offset} second') AT TIME ZONE ?) + INTERVAL '#{week_start} day' + INTERVAL '#{day_start} second') AT TIME ZONE ?", time_zone, time_zone]
          else
            ["(DATE_TRUNC('#{period}', (#{column}::timestamptz - INTERVAL '#{day_start} second' + INTERVAL '#{time_offset} second') AT TIME ZONE ?) + INTERVAL '#{day_start} second') AT TIME ZONE ?", time_zone, time_zone]
          end
        when "SQLite"
        ...
        when "Redshift"
          ...
          when :week # start on Sunday, not Redshift default Monday
            ...
            #
            ["CONVERT_TIMEZONE(?, 'Etc/UTC', DATE_TRUNC(?, CONVERT_TIMEZONE(?, #{column}) - INTERVAL '#{week_start} day' - INTERVAL '#{day_start} second'))::timestamp + INTERVAL '#{week_start} day' + INTERVAL '#{day_start} second' + INTERVAL '#{time_offset} second'", time_zone, period, time_zone]
          else
            ["CONVERT_TIMEZONE(?, 'Etc/UTC', DATE_TRUNC(?, CONVERT_TIMEZONE(?, #{column}) - INTERVAL '#{day_start} second'))::timestamp + INTERVAL '#{day_start} second' + INTERVAL '#{time_offset} second'", time_zone, period, time_zone]
          end
        else
